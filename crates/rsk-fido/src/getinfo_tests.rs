...
fn get_info_fields() {
    ...
    // prepends all three ML-DSA sets in descending security order, -87 (-50),
    // -65 (-49), -44 (-48) (default stays without them: Firefox
    // authenticator-rs strict parse).
    ...
    assert_eq!(
        ...
        3 + 3 * u64::from(pqc) + u64::from(eddsa)
    );
    let mut algs = vec![];
    if pqc {
        algs.push(ALG_MLDSA87);
...
/// `encIdentifier` (0x19) is emitted only when a persistent token exists, so its
/// presence moves the map count — the second conditional member after 0x0B. It is a
/// 2-byte key (25 > 23) and must land between 0x18 and 0x1A; getting that wrong
...
fn enc_identifier_is_conditional_and_sorts_between_its_neighbours() {
    ...
    // Walk to 0x19 and check both neighbours came in the right order.
    let mut keys = std::vec::Vec::new();
    for _ in 0..present {
        let k = d.u32().unwrap();
        keys.push(k);
        if k == 0x19 {
            assert_eq!(d.bytes().unwrap(), &blob, "0x19 carries iv ‖ ct verbatim");
        } else {
            d.skip().unwrap();
        }
    }
