...
pub fn next_free_retired<S: Storage>(fs: &mut Fs<S>) -> Option<u8> {
    (SLOT_RETIRED_FIRST..=SLOT_RETIRED_LAST).find(|&slot| {
        ...
        // out of twenty; taking `false` for it costs whatever the fault hid, because
        // the generate on the other side of this pick has no management-key auth.
        matches!(fs.try_has_key(key_fid(slot)), Ok(false))
            && cert_fid_for_slot(slot).is_none_or(|f| matches!(fs.try_has_data(f), Ok(false)))
    })
}
